...
def test_empate_de_score_lo_gana_la_mayor_capitalizacion() -> None:
    ...
    fin, carriles = select_finalists(prescored, held=set(), watch=[], per_sector=1, cap=2)
...
def test_sector_desconocido_no_recibe_plaza_garantizada() -> None:
    ...
    fin, carriles = select_finalists(prescored, held=set(), watch=[], per_sector=5, cap=1)
    # tope a 1: solo cabe la única plaza garantizada (T1 por sector); N1/N2 no tienen carril
    # propio (sector "n/d") y no queda hueco para que el resto por score los alcance
