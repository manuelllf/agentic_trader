...
Monta a mano una DB con las tablas tal como eran antes de entry_lane/had_prior_thesis, corre
`_migrate_books` y comprueba que las columnas aparecen, que los datos previos sobreviven, y que
la migración es idempotente (correrla dos veces no rompe nada). `news_used` NO se prueba aquí a
propósito: se normalizó a `ScoreNews` y ya no se crea (ver `_drop_columnas_muertas`, que la quita
si la encuentra en una BD vieja).
"""
...
def test_migracion_anade_columnas_y_conserva_datos(tmp_path):
    ...
    with engine.connect() as conn:
        ...
        row2 = conn.execute(text(
            "SELECT ticker, score, held FROM scores WHERE id=1"
        )).one()
